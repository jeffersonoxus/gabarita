...
export default async function HubPage() {
  ...
  const { data: concursos } = await sup.from("concursos").select("*").eq("ativo", true).order("created_at", { ascending: false });
            ...
            <CardContent className="flex items-center justify-between flex-wrap gap-4 p-5">
              <div>
                <p className="font-semibold flex items-center gap-2">
                  Plano Pro <Badge variant="default">R$ 14,90/mês</Badge>
                </p>
        ...
        {!concursos || concursos.length === 0 ? (
          <Card className="border-dashed">
            <CardContent className="py-12 text-center text-muted-foreground">
              ...
              <p className="text-xs mt-2">Execute o SQL de seed para adicionar dados de teste.</p>
            </CardContent>
          </Card>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {concursos.map((c: any) => (
              <Link key={c.id} href={`/simulado/${c.slug || c.id}`}>
                <Card className="hover:border-primary/50 hover:shadow-md transition-all duration-200 cursor-pointer group h-full">
                  ...
                  <CardContent className="pt-0">
                    <div className="flex items-center gap-3 text-xs text-muted-foreground mb-3">
                      ...
                      {c.data_prova && <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(c.data_prova + "T00:00:00").toLocaleDateString("pt-BR")}</span>}
                    </div>
                    <span className="text-xs font-medium text-primary flex items-center gap-1 group-hover:gap-2 transition-all">
                      Iniciar simulado <ArrowRight className="w-3.5 h-3.5" />
                    </span>
                  </CardContent>
                </Card>
              </Link>
